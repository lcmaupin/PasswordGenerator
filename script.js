...
function randomIndex(str){
  ...
  generateEl.addEventListener(`click`, () => {
    ...
    // generatePassword function takes booleans from checkboxes and length number and returns a string that is set to the innerText value of the result span
    resultEl.innerText = generatePassword(hasLower, hasUpper, hasNumber, hasSymbol, length);
  });

//   Copy password to clipboard
    clipboardEl.addEventListener(`click`, () => {
    // Creates a text area into which the password will be placed
    const textarea = document.createElement(`textarea`);

    // Sets password variable to string result
    const password = resultEl.innerText;

    // Prevents an empty string from being copied
    if (password === ``){
        alert(`Please generate a password first`);
        return;
    }

    // Sets the value for the textarea to the value of the current password
    textarea.value = password;

    // Selects the body
    const body = document.querySelector(`body`);

    // Adds textarea to the document
    body.append(textarea);

    // Uses select method to select text in textarea
    textarea.select();

    // Uses execCommand to copy the selecteed value to the clipboard
    document.execCommand(`copy`);

    // Removes created textarea from document
    textarea.remove();
